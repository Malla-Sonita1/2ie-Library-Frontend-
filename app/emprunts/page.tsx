...
export default function EmpruntsPage() {
  ...
  useEffect(() => {
    ...
    getUserLoans()
      .then((data) => setLoans(data.filter((l: any) => l.status === "active")))
  ...
  return (
    <div className="container-2ie py-8">
      <h1 className="text-2xl font-bold mb-4 flex items-center gap-2">
        <BookOpen className="h-6 w-6" /> Mes Emprunts
      </h1>
      {loading ? (
        <p>Chargement...</p>
      ) : loans.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loans.map((loan) => (
            <Card key={loan.id}>
              ...
              <CardContent>
                <p>Status: {loan.status === "active" ? "En cours" : loan.status}</p>
                {/* TODO: Ajouter bouton de retour si besoin */}
              </CardContent>
            </Card>
          ))}
        </div>
